...
(function(root, factory) {
  ...
  */
    ...
    $(document).on('pagebeforechange', function(e, data) {
      if (data.options.hasOwnProperty('_jqmrouter_handled')) {
        return;
      }
    ...
    if (config.fixFirstPageDataUrl) {
      $(document).ready(function() {
        ...
        var dataUrl = page.jqmData('url'),
          guessedDataUrl =
            window.location.pathname + config.firstPageDataUrl + window.location.search + window.location.hash;
    ...
    */
      this.routes = {
        ...
        pageinit: null,
        pageremove: null,
        pagebeforechange: null,
        pagebeforeload: null,
        pageload: null,
        popupbeforeposition: null,
        popupafteropen: null,
        popupafterclose: null
      };
      this.evtLookup = {
        bC: 'pagebeforechange',
        bl: 'pagebeforeload',
    ...
    $.extend($.mobile.Router.prototype, {
      ...
      add: function(userRoutes, userHandlers, _skipAttach) {
        ...
        this._detachEvents();
        var routeHandler = function(e, ui) {
          _self._processRoutes(e, ui, this);
        };
        if (evtList.length > 0) {
          this._eventData = {
            events: evtList.join(' '),
      ...
      _processRoutes: function(e, ui, page) {
        ...
        url = !this.conf.ajaxApp ? refUrl.hash : refUrl.pathname + refUrl.search + refUrl.hash;
        if (url.length == 0) {
        ...
        $.each(this.routes[e.type], function(route, routeConf) {
          ...
          if (bHandled && _self.conf.firstMatchOnly) {
            return false;
          }
        });
        ...
        if (bCData && e.isDefaultPrevented()) {
          bCData.deferred.done(function() {
            ...
            var extraOpt = bCData.toPage === ui.toPage ? { _jqmrouter_handled: true, _jqmrouter_bC: true } : null;
